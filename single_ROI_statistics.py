#!/usr/bin/env python3
"""
Single ROI Preference Analysis
...
def draw_single_roi(analyzer):
    ...
    """
    print("\n" + "="*60)
    print("SINGLE ROI PREFERENCE ANALYSIS")
    print("="*60)
    print("Draw ONE region of interest for focused analysis")
    print("\nControls:")
    ...
    def onkey(event):
        ...
        if event.key.lower() == 'c' and len(vertices) >= 3 and not roi_defined:
            ...
            # Get ROI name
            roi_name = input("\nEnter name for this ROI ( for instance 'target_zone'): ").strip()
            if not roi_name:
                roi_name = "target_roi"
            ...
            roi_defined = True
            ax.set_title('ROI defined - Close window to continue')
            ax.legend()
            plt.draw()
...
def calculate_preference_metrics(analyzer, roi_name, fps=25):
    """
    Calculate  behavioral preference metrics for single ROI
    """
    print("\n" + "="*60)
...
def analyze_single_roi():
    ...
    # Check for existing ROI definition
    ...
    print("\n1. PREFERENCE INDEX (Classic)")
    print("   PI = (T_in - T_out) / T_total × 100")
    print(f"   PI = ({metrics['time_in_roi_s']:.1f} - {metrics['time_outside_s']:.1f}) / {metrics['total_time_s']:.1f} × 100")
    print(f"   PI = {metrics['preference_index_classic']:.2f}%")
    
    print("\n2. DISCRIMINATION INDEX")
    print("   DI = (T_in - T_out) / (T_in + T_out)")
    print(f"   DI = ({metrics['time_in_roi_s']:.1f} - {metrics['time_outside_s']:.1f}) / ({metrics['time_in_roi_s']:.1f} + {metrics['time_outside_s']:.1f})")
    print(f"   DI = {metrics['discrimination_index']:.3f}")
    
    print("\n3. EXPLORATION RATIO")
    print("   ER = T_in / T_total")
    print(f"   ER = {metrics['time_in_roi_s']:.1f} / {metrics['total_time_s']:.1f}")
    print(f"   ER = {metrics['exploration_ratio']:.3f}")
    
    print("\n4. COHEN'S D EFFECT SIZE")
    print("   d = (P_observed - P_expected) / SD_expected")
    print(f"   d = ({metrics['percent_time_in_roi']/100:.3f} - {metrics['expected_percent_time']/100:.3f}) / SD")
    print(f"   d = {metrics['cohens_d_effect_size']:.3f}")
    ...
    print(f"Number of entries:    {metrics['number_of_entries']:.0f}")
    print(f"Entry frequency:      {metrics['entry_frequency_per_min']:.2f} per minute")
    print(f"First entry latency:  {metrics['first_entry_latency_s']:.1f} seconds")
    print(f"Mean visit duration:  {metrics['mean_bout_duration_s']:.2f} seconds")
    print(f"Max visit duration:   {metrics['max_bout_duration_s']:.2f} seconds")
    
    print("\nPREFERENCE INDICES:")
    print("-" * 40)
    print(f"Classic Index:        {metrics['preference_index_classic']:+.1f}%")
    print(f"Discrimination Index: {metrics['discrimination_index']:+.3f}")
    print(f"Exploration Ratio:    {metrics['exploration_ratio']:.3f}")
    ...
    print(f"Time vs chance:       p = {metrics['binomial_test_p']:.4f} {sig}")
    
    # Effect size interpretation
    if abs(metrics['cohens_d_effect_size']) > 0.8:
        effect = "Large effect"
    elif abs(metrics['cohens_d_effect_size']) > 0.5:
        effect = "Medium effect"
    elif abs(metrics['cohens_d_effect_size']) > 0.2:
        effect = "Small effect"
    else:
        effect = "Negligible effect"
    print(f"Effect size:          d = {metrics['cohens_d_effect_size']:.3f} ({effect})")
    ...
    if metrics['binomial_test_p'] < 0.05:
        if metrics['preference_index_classic'] > 0:
            print(f" SIGNIFICANT PREFERENCE for {roi_name}")
            print(f"  Animal spent {metrics['percent_time_in_roi'] - metrics['expected_percent_time']:.1f}% more time than expected by chance")
        else:
            print(f" SIGNIFICANT AVOIDANCE of {roi_name}")
            print(f"  Animal spent {metrics['expected_percent_time'] - metrics['percent_time_in_roi']:.1f}% less time than expected by chance")
    else:
        print(f"○ NO SIGNIFICANT PREFERENCE for {roi_name}")
        print(f"  Time spent matches chance expectation")
    ...
    print(f"2. {fig_filename}")
    
    print("\nANALYSIS COMPLETE, enjoy!")
