...
def get_tracking_bodypart(analyzer):
    """
    Detect the appropriate bodypart to use for tracking.
    Prioritizes: nose > Nose > body > first available bodypart
    """
    bodyparts = analyzer.bodyparts
    
    # Check for preferred bodyparts in order
    preferred_bodyparts = ['nose', 'Nose', 'body', 'Body']
...
def draw_single_roi(analyzer):
    ...
    # Get trajectory data - use first available bodypart or prefer nose/body
    bodypart = get_tracking_bodypart(analyzer)
    print(f"Using bodypart for visualization: {bodypart}")
...
def calculate_preference_metrics(analyzer, roi_name, fps=25):
    ...
    # Get appropriate tracking bodypart
    bodypart = get_tracking_bodypart(analyzer)
    print(f"Tracking bodypart: {bodypart}")
